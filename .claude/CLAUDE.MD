...
- Never store images as Base64.
- If the project later migrates to binary image storage, update this guide and the persistence layer together.
...
9. **No undocumented image storage changes** — if image persistence strategy changes, update this file and the persistence layer together.
10. **No blocking email sends** — always `@Async`.
11. **Immutability** — create new objects, never mutate existing ones. Domain objects from persistence are the single source of truth.
